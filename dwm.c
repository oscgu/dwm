...
#define XRDB_LOAD_STRING(NAME, DST)	\
	XrmGetResource(xrdb, NAME, NAME, &type, &value); \
	DST = value.addr;
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, ignoreconfigevent, noborder;
...
applyrules(Client *c)
{
	...
	for (i = 0; i < LENGTH(rules); i++) {
		...
		&& (!r->instance || strstr(instance, r->instance)))
		{
            ...
            c->ignoreconfigevent = r->ignoreconfigevent;
...
loadxrdb(void)
{
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        XRDB_LOAD_STRING("dwm.font", fonts[0]);
        XRDB_LOAD_STRING("dwm.fontbackup", fonts[1]);
...
setfocus(Client *c)
{
    ...
    if(c->ignoreconfigevent) setclientstate(c, NormalState);
